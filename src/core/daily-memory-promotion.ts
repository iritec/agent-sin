      ...
      messages: [
        {
          ...
          content: lLines(
            ...
            [
              "あなたはAgent-Sinの長期記憶キュレーターです。memory.mdはユーザー本人を深く理解し会話をスムーズにするための長期メモです。",
              "日別会話記録から、ユーザー理解に直結する観察だけをmemory.mdへ昇格します。基準は厳しく、迷ったら出さない。最大3件、ゼロ件で構わない。",
              "残す対象（ユーザー本人の理解に直結するもののみ）:",
              "- ユーザーの人物像・役割・経験・関心領域",
              "- コミュニケーション傾向、好む話し方、嫌う表現、判断の癖",
              "- 価値観・信条・大事にしていること",
              "- 通知やチャネルの嗜好、購読メディア、好む形式",
              "- 趣味・関心の薄い領域など、本人の固定的な志向",
              "残さない対象（運用ルール・スキル設定・作業ログは絶対に入れない）:",
              "- スケジュール（何時に何を通知するか）、通知先設定、フィルター条件、件数上限などスキルの動作仕様",
              "- 個別のメール・ファイル・ユーザー名に紐づくアクションルール",
              "- 直近タスクの作業内容、コミット、修正点、実装手順",
              "- その日限りの雑談、感想、近況、進捗報告、気分・体調・天気",
              "- ツール出力、ログ、コード片、URL列挙",
              "- 既存memory.mdと意味が重なる内容",
              "- 秘密情報、APIキー、トークン、機微な金融・家族情報、個人特定に至る他者情報",
              "出力規則: 1項目は1文の汎用ルール／事実として書く。日付・進行形・『今日〜した』表現を含めない。",
              '出力はJSONだけ: {"items":[{"text":"..."}]}。条件を満たすものがなければ {"items":[]} を返す。',
            ],
          ).join("\n"),
        },
        {
          role: "user",
          content: [
            l(`Target date: ${input.date}`, `対象日: ${input.date}`),
            "",
            "<existing-memory.md>",
      ...
      messages: [
        {
          ...
          content: lLines(
            [
              ...
              "- Preserve the existing heading structure when possible and normalize content under headings to one fact per bullet line starting with '-'. Do not create date headers or 'Auto promotion: ...' headings.",
            ...
            [
              "あなたはAgent-Sinの長期記憶整理係です。memory.mdはユーザー本人を深く理解するための長期メモであり、本文を読んで重複や陳腐化を整理し、新情報で既存項目を編集・上書きします。追記のみで肥大化させない。",
              "出力ルール:",
              "- マークダウンの本文だけを返す。前置き・後書き・コードフェンスを付けない。",
              "- 既存の見出し構成（## 仕事・関心 / ## コミュニケーション傾向 / ## 通知・メディア / ## 趣味・関心の薄い領域 など）を維持しつつ、各見出し配下を 1行1事実の箇条書き（- で始める）に統一する。日付ヘッダーや『自動昇格: …』の見出しは作らない。",
              "- 同じ意味の項目は1つに統合する。新情報が古い情報を更新している場合は古い方を捨て、最新の状態に書き換える。",
              "- ユーザー本人の理解に直結する観察（人物像・嗜好・価値観・コミュニケーション傾向・媒体の好み・趣味）は必ず残す。",
              "- 運用ルール・スケジュール・スキル動作の設定・特定の送信者名やフィルター条件・直近の作業内容・進捗・雑談はすべて捨てる。",
              "- 機微な金融情報・家族情報・他者を特定できる情報は残さない。",
              "- 内容を勝手に追加しない。元に無い事実は書かない。",
              "- 各行は1文の汎用ルール／事実として簡潔に書く。日付や『今日〜した』は含めない。",
              "- 全体で多くても 25 行程度に収める。減らせるなら減らす。",
              "整理対象が空または整理不要な場合でも、現状を保つ最小限の箇条書きを返す。",
            ],
          ).join("\n"),
        },
        {
